...
export default function FacultyLogin() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isRegister) {
      ...
      const newId = register(formData.name, formData.password, "faculty");
      setSuccessMessage(`Registration successful! Your Faculty Code is: ${newId}`);
      ...
      if (!formData.id || !formData.password) {
        setError("Please enter your Faculty Code and password");
        return;
      }
      const success = login(formData.id, formData.password, "faculty");
      if (success) {
        navigate("/faculty/dashboard");
      } else {
        setError("Invalid Faculty Code or password");
      }
    }
  };
        ...
        <div className="bg-card rounded-2xl shadow-2xl p-8 border border-border">
          ...
          <div className="text-center mb-8">
            ...
            <div className="w-24 h-24 mx-auto mb-4 bg-gradient-to-br from-accent/10 to-transparent border-2 border-accent/30 rounded-lg flex items-center justify-center p-4">
              <img src={facultyIcon} alt="Faculty Portal" className="w-full h-full object-contain" />
            </div>
            <h1 className="text-3xl font-bold text-foreground mb-2">Faculty Portal</h1>
            <p className="text-muted-foreground">
              {isRegister ? "Create your faculty account" : "Sign in to access your dashboard"}
            </p>
          </div>
              ...
              <div>
                ...
                <div className="relative">
                  <div className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5">
                    <img src={facultyIcon} alt="Faculty" className="w-full h-full object-contain opacity-60" />
                  </div>
                  <input
                    type="text"
                    id="name"
                    ...
                    className="w-full pl-10 pr-4 py-3 border border-border rounded-lg focus:outline-none focus:border-accent transition-colors duration-300 text-foreground"
          ...
          {!isRegister && (
            <div className="mt-6 p-4 bg-gray-50 rounded-lg">
              <p className="text-xs text-muted-foreground text-center">
                Demo: <span className="font-semibold">FA-1001</span> / <span className="font-semibold">faculty123</span>
              </p>
            </div>
          )}
        </div>
